...
from .constants import SESSION_PACKAGE_CHOICES
...
def get_secondary_group_for_player(primary_group, register_in_both_groups):
    """
    Return the other group if the admin chooses to register the player in both groups.

    Example:
    Primary group = Monday Group
    Secondary group = Saturday Group
    """
    if not register_in_both_groups or not primary_group:
        return None

    return TrainingGroup.objects.exclude(pk=primary_group.pk).first()
...
class PlayerForm(forms.ModelForm):
    ...
    register_in_both_groups = forms.BooleanField(
        required=False,
        label='Register in both groups',
        help_text='Tick this if the player can attend both Monday and Saturday groups.',
    ...
    class Meta:
        ...
        widgets = {
            ...
            'remaining_sessions': forms.NumberInput(attrs={
                'class': 'form-control',
                'min': 0,
            }),
            ...
            'plan_end_date_manual': forms.DateInput(attrs={
                'class': 'form-control',
                'type': 'date',
            }),
            'plan_sessions_count': forms.NumberInput(attrs={
                'class': 'form-control',
                'min': 0,
            }),
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Player code exists in both create and edit forms.
        if 'player_id' in self.fields:
        ...
        # These fields exist only in the edit form, not in PlayerCreateForm.
        if 'plan_end_date_manual' in self.fields:
            self.fields['plan_end_date_manual'].disabled = True
            self.fields['plan_end_date_manual'].help_text = (
                'End date is calculated automatically from start date, group selection, and current plan sessions.'
            )

        # Load username from linked Django user.
        if 'username' in self.fields:
            if self.instance and self.instance.pk and self.instance.user:
                self.fields['username'].initial = self.instance.user.username

        # If player already has secondary group, mark both-groups checkbox.
        if 'register_in_both_groups' in self.fields:
            if self.instance and self.instance.pk:
                self.fields['register_in_both_groups'].initial = bool(self.instance.secondary_group)
    ...
    def clean(self):
        ...
        register_in_both_groups = cleaned_data.get('register_in_both_groups')
        ...
        if plan_start_date and plan_sessions_count > 0 and group:
            secondary_group = get_secondary_group_for_player(
                group,
                register_in_both_groups,
            )
            ...
            temp_player.secondary_group = secondary_group
        ...
        elif 'plan_end_date_manual' in self.fields:
    ...
    def save(self, commit=True):
        ...
        register_in_both_groups = self.cleaned_data.get('register_in_both_groups')

        if not player.player_id or player.player_id == 'Auto generated':
            player.player_id = generate_unique_player_code()

        player.secondary_group = get_secondary_group_for_player(
            player.group,
            register_in_both_groups,
        )
...
class PlayerCreateForm(PlayerForm):
    """
    Admin form for creating a player only.

    Sessions are not added here.
    The player starts with 0 sessions, then admin adds sessions from Add Sessions page.
    """
    ...
    def save(self, commit=True):
        # Do not call PlayerForm.save() here.
        # We want to create only the player account, with zero sessions.
        player = forms.ModelForm.save(self, commit=False)
        ...
        player.remaining_sessions = 0
        player.plan_start_date = None
        player.plan_end_date_manual = None
        player.plan_sessions_count = 0
...
class PlayerSelfRegistrationForm(forms.Form):
    """
    Old self-registration form.

    This can stay here safely, but your current flow should redirect /register/
    to player login because admins now create player accounts.
    """
